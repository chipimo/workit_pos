        ...
        switch (props._type) {
                ...
                switch (props.layoutType) {
                    ...
                    case "ProductsList":
                        ...
                        dbhook
                            ...
                            .then(function (data) {
                            var resultData = data.map(function (list) {
                                list.totalQty = list.amountInstore;
                                if (!list.isExpired)
                                    newArr4_1.push(list);
                                return newArr4_1;
                            });
                    ...
                    if (list.isMulity) {
                        ...
                        dbhook("products")
                            ...
                            dbhook("balancesReports")
                                .where({ name: list.productKey })
                                .then(function (closingProductsList) {
                                if (closingProductsList.length === 0)
                                    dbhook("balancesReports")
                                        .insert({
                                        idKey: uuidv1(),
                                        name: list.productKey,
                                        date: moment().format("MM/DD/YYYY"),
                                        DateRange: parseInt(DateNumInput),
                                        OpenBalance: productsList[0].amountInstore,
                                        CloseBalance: productsList[0].amountInstore - list.qnt,
                                        QuantitySold: list.qnt,
                                    })
                                        .then(function (dataRe) { });
                                else if (closingProductsList[0].DateRange === parseInt(DateNumInput))
                                    dbhook("balancesReports")
                                        .where({ name: list.productKey })
                                        .update({
                                        CloseBalance: closingProductsList[0].CloseBalance - list.qnt,
                                        QuantitySold: closingProductsList[0].QuantitySold + list.qnt,
                                    })
                                        .then(function (data) { });
                                else
                                    dbhook("balancesReports")
                                        ...
                                        dbhook
                                            ...
                                            if (Data.length !== 0)
                                                dbhook("balancesReports")
                                                    ...
                                                    QuantitySold: Data[0].OpenBalance -
                                                        productList.amountInstore,
                                                })
                                                    .then(function () { });
                                            else
                                                dbhook("balancesReports")
            ...
            case "getBalancesReports":
                dbhook
                    ...
                    .from("balancesReports")
                    .whereBetween("DateRange", [props.startDate, props.endDate])
                    .innerJoin("products", "balancesReports.name", "products.productKey")
                    .then(function (data) {
                    console.log(data);
                    sendCallback(data);
                });
                break;
            case "setBalancesReports":
            ...
            case "bulkUpload":
                bulkuploadPromises = props.data.map(function (list) {
                    return new Promise(function (resolve, reject) {
                        dataBase_1.default.HandleBranches({ type: "get" }, function (branch) {
                            exports.GetData({
                                table: "products",
                                id: "ItemName",
                                value: list.Product,
                                    ...
                                    exports.GetData({
                                        ...
                                        if (TabCallback.data.length !== 0) {
                                            dbhook("products")
                                                .insert({
                                                productKey: uuidv1(),
                                                group: TabCallback.data[0].id,
                                                category: list.Category,
                                                ItemName: list.Product,
                                                barcode1: list.barcode.toString(),
                                                barcode2: "",
                                                barcode3: "",
                                                barcode4: "",
                                                barcode5: "",
                                                branches: branch[0].branche,
                                                supplier: list.Supplier,
                                                ingredient: "",
