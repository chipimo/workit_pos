...
export const HandelNewProducts = async (props, dbhook, sendCallback) => {
  ...
  switch (props._type) {
    ...
    case "addServerProducts":
      const { alltabs, allproductsList, allmulitList, socketId } =
        props.AllProducts;
              ...
              dbhook("balancesReports")
                ...
                .then((closingProductsList) => {
                  if (closingProductsList.length === 0)
                    ...
                    closingProductsList[0].DateRange === parseInt(DateNumInput)
                  )
                    dbhook("balancesReports")
                      .where({ name: list.productKey })
                      .update({
                        CloseBalance:
                          closingProductsList[0].CloseBalance - list.qnt,
                        QuantitySold:
                          closingProductsList[0].QuantitySold + list.qnt,
                      })
                      .then(function (data) {});
                  else
                    dbhook("balancesReports")
    ...
    case "bulkUpload":
      let bulkuploadPromises = props.data.map((list) => {
        return new Promise(function (resolve, reject) {
          appDb.HandleBranches({ type: "get" }, (branch) => {
            GetData(
              {
                table: "products",
                id: "ItemName",
