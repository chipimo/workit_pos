...
import PrintToDisplay from "./serialport";
import clearDisplay from "./serialport";
    ...
    db.createTable("config", (succ, msg) => {
      db.getAll("config", (succ, data) => {
        ...
        if (data.length !== 0) {
          ...
          nodemon({
            script: path.join(__dirname, "workit-server/index.js"),
            ext: "js json",
          });
...
ipcMain.on("printToDisplay", (event, arg) => {
  PrintToDisplay(arg)
});
...
async function exportFromMain(props): Promise<void> {
  ...
  const file = `${userhome}/Documents/Workit-Tec/reports/doc.md`;

  fs.ensureFileSync(file);
  const pdfPathMain = `${userhome}/Documents/Workit-Tec/reports/financial-Report-${CreateId()}.pdf`;
